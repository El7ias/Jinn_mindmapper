/**
 * OrchestrationEngine — Session lifecycle manager for Claude Code execution.
 *
 * Ties together the full pipeline:
 *   Canvas → MindMapSerializer → WorkflowPromptGenerator → ClaudeCodeBridge
 *
 * State machine:
 ...
 *                                   → cancelled
 *
 * Events consumed from ClaudeCodeBridge (via EventBus):
...
export class OrchestrationEngine {
  ...
  constructor(bus, managers) {
    ...
    this._bridge = new ClaudeCodeBridge(bus);
  ...
  async startSession(options = {}) {
    ...
    try {
      ...
      // 3. Execute via ClaudeCodeBridge
